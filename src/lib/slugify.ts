...
const REGEX_INVALID_CHARS = /[^\w\s$*_+~.()'"!\-:@]+/g;
const REGEX_ALPHANUMERIC = /[^A-Za-z0-9\s]/g;
const REGEX_SPACE = /\s+/g;

function resolveOptions(optionsOrReplacement?: SlugifyOptions | string) {
  if (typeof optionsOrReplacement === "string") {
    return { replacement: optionsOrReplacement } satisfies SlugifyOptions;
  }
  return optionsOrReplacement ?? {};
}
...
const defaultOptions = {
  ...
  locale: "vi",
  /** Trim leading and trailing replacement chars */
  trim: true,
...
export function slugify(
  ...
  const opts = {
    ...defaultOptions,
    ...resolveOptions(optionsOrReplacement),
  } satisfies SlugifyOptions;

  const locale = opts?.locale ? LOCALES[opts.locale] : {};

  const chars = text.normalize().split("");
  let slug = chars.reduce((acc, char) => {
    let c = locale[char] ?? REPLACE_CHARS[char] ?? char;
    if (c === opts.replacement) {
      c = " ";
    }
    return acc + c.replace(opts.remove ?? REGEX_INVALID_CHARS, "");
  }, "");

  if (opts.strict) {
    slug = slug.replace(REGEX_ALPHANUMERIC, "");
  }
  if (opts.trim) {
    slug = slug.trim();
  }
  slug = slug.replace(REGEX_SPACE, opts.replacement);
  if (opts.lower) {
...
export function createSlug(prefix: string) {
  return slugify(`${prefix}-${nanoid(4)}`);
}
